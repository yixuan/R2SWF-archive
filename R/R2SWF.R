...
##' convert them to a Flash file (.swf). Supported input formats are
...
##' }
##'
image2swf <- function(filenames, outdir = getwd(), output = "movie.swf",
					  interval = 1)
{
	# The formats of files. 1 for png, 2 for jpg/jpeg, and 0 for others.
	fmt = rep(0, length(filenames));
	fmt[grep("\\.[Pp][Nn][Gg]$", filenames)] = 1;
    fmt[grep("\\.[Jj][Pp][Ee]?[Gg]$", filenames)] = 2;
	
	oldwd = setwd(outdir);
    .Call("image2swf", as.character(filenames), as.integer(fmt),
	      as.character(output), as.numeric(interval),
		  PACKAGE = "R2SWF");
	setwd(oldwd);
					...
					img.name = "Rplot", ...)
{
    ...
    output = image2swf(files, outdir, output, interval)
	setwd(olddir)
